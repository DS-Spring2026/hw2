...
In this assignment, you will be using Databricks on Google Cloud Platform (GCP), creating a table with data from the yellow cab NYC Taxi Dataset [NYC Taxi Data](https://www.nyc.gov/site/tlc/about/tlc-trip-record-data.page), and then running a set of SQL queries against that table. You will then import this same data into BigQuery and run the same queries against that table. You will measure the runtimes of each and compare the performance of the SQL workload on BigQuery and Databricks.
...
You will be brought to a page where you are asked to upload files. Upload the yellow-cab data Parquet files you downloaded for February through December 2024 you downloaded earlier from the NYC taxi site into a single table. Once these are done uploading, you'll be taken to the following page, where you can rename the table under Table Name.
...
After the setup, import data from February to December 2024 yellow-cab Parquet files into a single BigQuery **INTERNAL** table. 
...
## SQL Workload 
Now that we have this setup, we're going to run a SQL workload in Databricks. There are 4 SQL query files in `BigQuery` and `Databricks` folder in your repo. You're going to run each query first against the Databricks table, and then against both BigQuery table versions (using standard, built-in tables that are stored internally as well as tables which are externally stored). You can copy and paste the queries into the editors, or read documentation online to setup APIs to execute SQL queries via APIs.
